...
Claster* Gkk::get_membership_matrix()
{
    Claster* c = new Claster();
    for (int i=0; i<this->membership_matrix.size(); ++i)
    {
        c->objects.push_back(this->membership_matrix[i]);
    }
    if (membership_matrix.size() != 0)
        c->propnum = membership_matrix[0].size();
...
Claster* Gkk::get_distances_matrix()
{
    this->distance_matrix.clear();
...
void Gkk::set_memberships_byrandom(int numofclusters, int numofobj)
{
...
vector< vector< float > > Gkk::V(int l, float Y)
{
...
void Gkk::countmembership(int clast_num, float E, float Y)
{
    float sum , max;   
    vector<float> center;
...
vector< Claster* > Gkk::clustering(int clast_num, float E, float Y)
{
    float max;
